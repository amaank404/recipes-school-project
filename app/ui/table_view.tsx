...
export function RecipeTableItem({
  ...
  recipe: Recipe;
  className?: string;
  onData?: (id: string, checked: boolean) => void;
  checked?: boolean;
}) {
  return (
...
function NoRowsPlaceholder() {
  return (
    <div className="w-full h-full flex items-center flex-col text-slate-400 py-20">
...
export default function TableView({
  ...
  className?: string;
  data: Recipe[];
  onData?: (checked_items: string[]) => void;
}) {
  ...
  let rows = data.map((recipe: Recipe) => (
    <RecipeTableItem
      recipe={recipe}
      key={recipe.id}
